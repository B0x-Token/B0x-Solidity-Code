...
if (BigInt(addr1) > BigInt(addr2)) {
  ...
  gasPrice: doubledGasPrice
    ...
    // Print verification command
    console.log(`\nTo verify on Etherscan, run:`);
  console.log(`npx hardhat verify --network baseSepolia ${tokenAddress_Rewards} "${tokenAddress}" "${Address_ZEROXBTC_TESTNETCONTRACT}" "${HookAddress}" \n\n`);
...
console.log(`Estimated Deposit 0xBTC amount: ${ethers.formatUnits(amountWith8Decimals0xBTC, 8)}`); // 10.0
     ...
     try {
      //1000 is 1% slippage
              const tx = await tokenSwapper.createPositionWith2Tokens(tokenAddress, Address_ZEROXBTC_TESTNETCONTRACT, amountToDeposit,amountWith8Decimals0xBTC,Current_getsqrtPricex96, 1000, HookAddress, deployer.address, {
                gasPrice: doubledGasPrice
                });
